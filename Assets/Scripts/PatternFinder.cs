...
{
    //a unility class for finding the patterns and it's neighbour from the tilemap
    //used by PatternManager and NeughbourStartegy Classes
    public static class PatternFinder
    {

        public static  PatternDataResults GetPatternDataFromGrid(TileMapReader reader, int patternSize, bool equalWeights)
        {
            ...
            Vector2 sizeOfGrid = reader.GetGridSize();
            ...
            for (int row = rowMin; row < rowMax; row++)
            {
                ...
                {
                    int[][] gridValues = reader.GetPatternValuesAt(col, row, patternSize);
                    ...
                    if (patternHashcodeDictionary.ContainsKey(hashValue) == false)
                    {
                        ...
                        //patternIndex++;
                        AddNewPattern(patternHashcodeDictionary, patternIndexDictionary, hashValue, gridValues, patternIndex);
                        patternIndex++;
                    }
                    else
                    {
        ...
        public static int GetPatternGridSize(int patternSize,int sizeOfGrid) {
            int patternGridSize = 0;
            if (patternSize < 3)
            {
                patternGridSize = sizeOfGrid + 3 - patternSize;
                
            }
            else
            {
                patternGridSize = sizeOfGrid + patternSize - 1;
              
            }

            return patternGridSize;
        ...
        public static Dictionary<int, PatternNeighbours> FindPossibleNeighbursForAllPatterns(IFindNeighbourStrategy patternFinder, PatternDataResults patterndataResults)
        {
        ...
        private static void AddNewPattern(Dictionary<string, PatternData> patternHashcodeDictionary, Dictionary<int, PatternData> patternIndexDictionary, string hashValue, int[][] gridValues, int patternIndex)
        {
            PatternData patternData = new PatternData(gridValues, hashValue, patternIndex);
            patternHashcodeDictionary.Add(hashValue, patternData);
            patternIndexDictionary.Add(patternIndex, patternData);
        }
